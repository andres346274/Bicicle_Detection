function y = ExtraeCaracteristicas(M)
    
    M = im2double(M);
    
    % Compruebo el tamaï¿½o de mi imagen para determinar si esta en blanco y negro y RGB
    size_image = size(M);
    ...
    % Cuerpo de la bici y segmento que une los centros a 35ï¿½-55ï¿½ o -35ï¿½-55ï¿½ 
    ...
    y=[numero_de_ruedas_detectado; diferencia; media_de_numero_lineas; contador_lineas_angulo];
    
    % Para asegurar que la normalizacion no va a resultar en un error
    for i=1:length(y)
        if y(i)==0
            y(i)=0.00001;
        end
